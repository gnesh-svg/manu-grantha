...
filter_strength = st.sidebar.slider("Filter Strength (1-25 px)", 1, 25, 5)
...
window_size = st.sidebar.slider("Local Window Size (3-101 px)", 3, 101, 25, step=2)

# 4. Canny Section
st.sidebar.subheader("3. Canny Sensitivity")
edge_threshold = st.sidebar.slider("Edge Threshold (10-250)", 10, 250, 100)
...
if uploaded_file is not None:
    ...
    # 🔭 Zoom Inspector
    st.markdown("---")
    st.subheader("🔬 Character Inspector (Zoom)")
    z_col1, z_col2 = st.columns([1, 2])
    
    with z_col1:
        h, w = final_binary.shape
        x = st.slider("X Coordinate", 0, w, w//2)
        y = st.slider("Y Coordinate", 0, h, h//2)
        zoom_level = st.select_slider("Zoom Level", options=[1, 2, 3, 4], value=2)
    
    with z_col2:
        crop_size = 100 // zoom_level
        y1, y2 = max(0, y-crop_size), min(h, y+crop_size)
        x1, x2 = max(0, x-crop_size), min(w, x+crop_size)
        zoom_img = final_binary[y1:y2, x1:x2]
        st.image(zoom_img, caption=f"Zoomed view at ({x}, {y})", width=400)
    ...
    st.info("Upload a manuscript image to begin research analysis.")
