...
# --- Page Config ---
st.set_page_config(page_title="ManuscriptMaster: Research Edition", layout="wide")

st.title("📜 ManuscriptMaster: Web Edition")
st.markdown("---")

# --- Initialize Session State ---
if 'view_mode' not in st.session_state:
    st.session_state.view_mode = "Comparison View"
...
def reset_params():
    ...
    st.session_state.clahe_val = True
...
use_clahe = st.sidebar.checkbox("Enable CLAHE Enhancement", key="clahe_val")
...
# 5. System Operations
st.sidebar.markdown("---")
if st.sidebar.button("🔴 Terminate Application", help="Stops the local server process"):
    st.warning("Server is shutting down. You can now close this tab.")
...
if uploaded_file is not None:
    ...
    with st.container(border=True):
        st.subheader("🔬 Comprehensive Experiment Log")
        col_log1, col_log2 = st.columns(2)
        
        with col_log1:
            st.markdown(f"**Preprocessing Configuration**")
            st.write(f"• Filter: `{filter_type}`")
            st.write(f"• Filter Strength: `{filter_strength} px`")
            st.write(f"• CLAHE Enabled: `{'Yes' if use_clahe else 'No'}`")
            st.write(f"• Canny Threshold: `{edge_threshold}`")

        with col_log2:
            st.markdown(f"**Binarization & Results**")
            st.write(f"• Strategy: `{thresh_type}`")
            st.write(f"• Window Size: `{window_size} px`")
            st.write(f"• **MSE:** `{mse:,.1f}` | **PSNR:** `{psnr:,.2f} dB`")
